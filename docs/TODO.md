...
- T37 app.py 装配层契约测试（PR #150，23 用例）：静态页路由（/ 前端入口+缺失回退 API 存活、/assets 静态资源、/favicon.ico 缺失 404、/vite.svg、/changelog 正常+缺失+回退 BASE_DIR，测试自建临时 dist 适配 CI 无构建产物）；_get_db_path env 双分支；_ensure_db 缺失→init 调用链/损坏→init/init 异常静默；_after_publish 200+taskId 跳过/200 无 taskId failed/非 200 failed+msg/坏 JSON HTTP<code>/非 postVideo no-op；/api/health 隔离库 db_ok+user_info 计数/损坏 db_error；find_available_port 正常/占用跳过/全占用 RuntimeError；模块级 subprocess 验证 ALL_PROXY 清除 → app.py 52%→70%（剩余 107 行均为 __main__ 启动段）；全量 3017→3040 passed + 12 skipped、总覆盖率 83%（CI 门槛 34%，裕度 49pp）
- T38 三平台 DOM 契约测试（PR #151，351 用例）：test_channels_platform_dom.py 182 用例（上传/填写/页面设置/视频标注/封面 21 分支/定时/提交发布 + login/check_cookie/sync_profile/_scrape_channels_stats/open_creator_center/publish_video 剩余分支）→ impl/channels/platform.py 19%→100%；test_jingmai_platform_dom.py 24 用例（login URL 回跳/check_cookie 有效失效+load_state close 防御/sync_profile 全分支/_scrape_stats 主帧+子帧兜底+JS 异常/_build_stats 千分位小数/publish_video 委托 jd/open_creator_center 线程+close 防御）→ impl/jingmai/platform.py 19%→100%；test_taobao_guanghe_platform_dom.py 145 用例（_group_by_trace/_replay_groups/_legacy_link_by_title 关联组重现、login/check_cookie/sync_profile、_upload_single_video 全流程+dry_run+异常吞掉、_set_cover file chooser/直注入、_fill_desc_and_tags 标签预算、_set_schedule_time JS/兜底 radio、publish_video 多文件×多账号循环+shop/字符串规范化+方向封面）→ impl/taobao_guanghe/platform.py 20%→100%；三文件合并 2101/2101=100%；全量 3040→3391 passed + 12 skipped、总覆盖率 83%→88%（CI 门槛 34%，裕度 54pp）
- T36 ext_api 应用层契约测试 + 防御性修复（PR #149，116 用例）：test_ext_api_tasks_domain.py 36 用例（任务域 get_tasks 分页/status 过滤/坏 JSON 兜底、create_task 空 body+5 必填字段逐一缺失 400+未知平台「未知」+全参数映射、get_task 404/500、cancel/retry 200/400 双分支、task_stream SSE data/heartbeat/GeneratorExit 订阅清理/queue.Full 吞掉、queue_status；task_queue _build_account_configs 全字段/add_task 自动 start/get_status/_notify_status 回调异常吞掉）；test_ext_api_stats_drafts_misc.py 80 用例（get_stats 空库+状态计数+平台聚合+7 天趋势+500、settings 默认合并+类型转换+坏 JSON 兜底+reset_storage+400/500、草稿 CRUD 全链路+重算 channels_summary+提取 helper 族全分支、get_changelog 目录缺失+日期解析+倒序、publish-templates type/页码校验+cover 回落+坏 image_ids+page_size 截断、batch_delete_drafts 400/混合/删除异常、历史域 time_range 三档+过滤+include_legacy+_normalize_detail_row duration+平台校正(拼音 key/未知+DB)+get_history_batch 404/500+batch/单条删除全分支、_to_beijing_time 坏输入+_resolve_cover_url/from_path 全路径+_ensure_tables 异常静默幂等）；顺带修复 compute_personalized 非 dict 值崩溃（_id_of 防御，行为等价）→ ext_api/__init__.py 50%→99%、task_queue.py 62%→69%、_personalized.py 53%→95%；全量 2901→3017 passed + 12 skipped、总覆盖率 81%→83%（CI 门槛 34%，裕度 49pp）
