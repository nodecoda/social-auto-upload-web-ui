"""B站 publish_video 编排层契约测试（T21）。

publish_video(同步) 内联 async _run(): 封面仅横版(landscape→thumbnail_path) →
排期(标量兜底) → 文件×账号笛卡尔积 → _upload_single_video。
特有: category / bili_repost_source / bili_collection_name / creation_declaration。
"""
import sys
...
def _run_publish(platform, **kwargs):
    ...
    with patch.object(platform, '_upload_single_video', upload), \
         patch('impl.bilibili.platform.parse_schedule_time', pst), \
         patch('impl.bilibili.platform.get_account_name_by_cookie_file', return_value='昵称'), \
         patch('impl.bilibili.platform.bind_account_name', MagicMock()):
...
class TestPublishVideoOrchestration:
    ...
    def test_landscape_cover_used_portrait_ignored(self):
        """B站只用横版封面,竖版封面被忽略。"""
        inst = _make_platform()
    ...
    def test_param_passthrough(self):
        ...
        _, upload, _ = _run_publish(
            ...
            desc='描述', category=2, creation_declaration='转载',
            bili_repost_source='https://example.com/src', bili_collection_name='合集A',
        )
        call = upload.await_args
        assert call.kwargs['title'] == '标题'
        assert call.kwargs['tags'] == ['t1']
        assert call.kwargs['desc'] == '描述'
        assert call.kwargs['category'] == 2
        assert call.kwargs['creation_declaration'] == '转载'
        assert call.kwargs['bili_repost_source'] == 'https://example.com/src'
        assert call.kwargs['bili_collection_name'] == '合集A'
    ...
    def test_strategy_log_immediate(self):
        inst = _make_platform()
        logger = MagicMock()
        with patch('impl.bilibili.platform.logger', logger), \
             patch.object(inst, '_upload_single_video', AsyncMock()), \
             patch('impl.bilibili.platform.parse_schedule_time', MagicMock(return_value=[None])), \
             patch('impl.bilibili.platform.get_account_name_by_cookie_file', return_value='昵称'), \
             patch('impl.bilibili.platform.bind_account_name', MagicMock()):
