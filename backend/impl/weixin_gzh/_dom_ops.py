"""微信公众号平台 — 视频发布 DOM 交互/表单构造/定时选择 子模块（A8 拆分）。

从 platform.py 拆出的平台专属 DOM 操作: 原 WeixinGzhPlatform 的 staticmethod,
现为模块级函数, 由 platform.py 以 `_x = staticmethod(_x)` 类属性绑定,
保持 `self._x(...)` / `WeixinGzhPlatform._x(...)` 调用语义不变(零行为变更)。
"""
import asyncio
import os
import re
...
from .._utils import clear_input, parse_schedule_time
...
async def _set_cover(page, cover_path: str):
        ...
        # 3. 等「下一步」按钮可点后点击
        await _click_primary_when_enabled(
...
async def _set_collection(page, collection_name: str):
        ...
        # 4. 点「确认」
        await _click_dialog_primary(page, "确认")
...
async def _set_claim_source(page, claim_source: str):
        ...
        await _click_dialog_primary(page, "确认", timeout_s=15)
...
async def _publish_immediate(page):
        ...
        await _click_dialog_primary(page, "发表", timeout_s=15)
        logger.info("[阶段②] 已点击弹窗「发表」,等待确认...")

        # 3. 二次确认弹窗「继续发表」(若有)
        await asyncio.sleep(2)
        await _click_dialog_primary(page, "继续发表", timeout_s=15)
        logger.info("[阶段②] 已点击「继续发表」,等待跳转首页...")

        # 4. 二次确认后公众号会弹扫码确认(管理员微信扫码),扫码通过后页面才跳转。
        #    这里只等页面跳转到首页/成功页,不点任何按钮 —— 扫码是用户线下动作。
        #    给 10 分钟供用户扫码。
        await _wait_for_home(page, timeout_s=600)
...
async def _publish_scheduled(page, publish_dt):
        ...
        **重要(实测)**: 发表弹窗里有两个开关:
        ...
        target_date_label = _resolve_date_label(publish_dt)
        await _select_schedule_date(page, target_date_label)

        # 4. 选择时间(时/分滚轮)
        target_hour = publish_dt.strftime("%H")
        target_minute = publish_dt.strftime("%M")
        await _select_schedule_time(page, target_hour, target_minute)
...
async def _select_schedule_time(page, hour: str, minute: str):
        ...
        **关键坑(定时时间一直选不上的真正根因)**:
        ...
        FIND_DL = _find_visible_picker_dl_js()
        ...
        h_ok = await _click_time_wheel_item(
            page, "hour", hour
        )
        ...
        m_ok = await _click_time_wheel_item(
            page, "minute", minute
        )
...
def _wheel_items_js_body(kind: str) -> str:
        ...
        return (
            "const dl = (" + _find_visible_picker_dl_js() + ")();"
...
async def _click_time_wheel_item(page, kind: str, value: str) -> bool:
        ...
        """
        body = _wheel_items_js_body(kind)
        ...
        await asyncio.sleep(0.3)
        sel1 = await _is_wheel_item_selected(page, kind, value)
        ...
        sel2 = await _is_wheel_item_selected(page, kind, value)
