...
import { useImageBatchSetApply, type BatchPanelIndex } from './useImageBatchSetApply'

// xiaohongshu key → name '小红书' (来自 @/config/platforms 真实注册表)
function setup() {
  const panels: BatchPanelIndex = {
    xiaohongshu: {
...
describe('useImageBatchSetApply', () => {
  ...
  it('scheduleTime 非空时 enableTimer 派生为 true', () => {
    ...
    expect(panels.xiaohongshu!.publicApi!.setPlatformConfig).toHaveBeenCalledWith(expect.objectContaining({ enableTimer: true }))
  })

  it('scheduleTime 留空时 enableTimer 派生为 false', () => {
    const { panels, applyImageBatchSet } = setup()
    applyImageBatchSet(['xiaohongshu'], { title: 't', description: 'd', tags: [], scheduleTime: '' })
    expect(panels.xiaohongshu!.publicApi!.setPlatformConfig).toHaveBeenCalledWith(expect.objectContaining({ enableTimer: false }))
  })
  ...
  it('tags 拷贝隔离: 不修改调用方数组', () => {
    ...
    applyImageBatchSet(['xiaohongshu'], { title: 't', description: 'd', tags, scheduleTime: '' })
    tags.push('y')
    expect(panels.xiaohongshu!.publicApi!.setPlatformConfig).toHaveBeenCalledWith(expect.objectContaining({ tags: ['x'] }))
  })
