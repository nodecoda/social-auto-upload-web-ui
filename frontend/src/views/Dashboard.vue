<template>
  <div class="dashboard">
    ...
    <div class="stat-cards">
      <StatCard variant="purple" :value="accountStats.total" label="账号总数"
        :details="[
        ...
        <template #extra>
          <button class="batch-check-btn" @click="handleBatchCheck" :disabled="isChecking">
            ...
            <template v-else><el-icon><Refresh /></el-icon>批量检查</template>
          </button>
        </template>
      </StatCard>
      <StatCard variant="blue" :value="platformStats.total" label="已接入平台">
        <template #icon><el-icon><Platform /></el-icon></template>
        <template #bottom>
          <!-- 参考 DraftBox.channels 跑马灯: 溢出时横向滚动 -->
          <div class="platform-channels">
            <div class="channels-track" :class="{ 'channels-marquee': platformOverflow }" ref="platformTrackRef">
              <span v-for="p in sortedPlatforms" :key="p.id" class="channel-tag" :class="{ 'is-active': p.count > 0 }">
                <img v-if="getPlatformLogo(p.key)" :src="getPlatformLogo(p.key) || undefined" :alt="p.name" class="channel-icon" />
      ...
      <StatCard variant="cyan" :value="contentStats.total" label="素材总数"
        :details="[
      ...
      <StatCard variant="green" value="—" label="今日发布" :details="[{ label: '成功率', value: '—' }]">
    ...
    <div class="quick-actions">
      <QuickActionCard variant="purple" title="快速发布" desc="发布内容到各平台" route="/publish-center" @navigate="navigateTo">
        <template #icon><el-icon><Upload /></el-icon></template>
      </QuickActionCard>
      <QuickActionCard variant="blue" title="上传素材" desc="上传和管理视频素材" route="/material-management" @navigate="navigateTo">
        <template #icon><el-icon><Document /></el-icon></template>
      </QuickActionCard>
      <QuickActionCard variant="cyan" title="系统设置" desc="配置系统参数和选项" route="/settings" @navigate="navigateTo">
        <template #icon><el-icon><Setting /></el-icon></template>
      </QuickActionCard>
      <QuickActionCard variant="green" title="账号管理" desc="管理所有平台账号" route="/account-management" @navigate="navigateTo">
    ...
    <RecentMaterialsTable
      ...
      :loading="loading"
      @view-all="navigateTo('/material-management')"
    />
  </div>
</template>
...
import RecentMaterialsTable, { type MaterialItem } from '@/components/RecentMaterialsTable.vue'
...
.dashboard {
  ...
  // materials-card 样式已迁移至 components/RecentMaterialsTable.vue
}
</style>
